...
fn main() {
    println!("{:?}", part_one(load_input("src/input")));
    println!("{:?}", part_two(load_input("src/input")));
}
...
fn pre_process(value: String) -> String {
    let mut prevous_end: usize = 0;
    let regex: Regex = Regex::new(r"don't\(\).*?do\(\)").unwrap();
    let mut clean_str: String = String::with_capacity(value.len());

    for mat in regex.find_iter(value.as_bytes()) {
        if prevous_end == 0 {
            clean_str.push_str(&value[0..mat.start()]);
            println!("Appending {:?}\n", &value[0..mat.start()]);
            println!("skipping {:?}\n", &value[mat.start()..mat.end()]);
            prevous_end = mat.end();
            continue;
        }
        clean_str.push_str(&value[prevous_end..mat.start()]);
        println!("appending {:?}\n", &value[prevous_end..mat.end()]);
        println!("skipping {:?}\n", &value[mat.start()..mat.end()]);
        prevous_end = mat.end();
    }

    clean_str.push_str(&value[prevous_end..]);
    println!("appending {:?}\n", &value[prevous_end..]);
...
mod tests {
    ...
    #[test]
    fn test_pre_process(){
        assert_eq!(pre_process(String::from("this don't()sadfasdfasdfsdafdo()is a don't()asdfasdfdon't()do()test")), "this is a test");
    }

    #[test]
    fn test_pre_process_02(){
        assert_eq!(pre_process(String::from("don't()Do()asdfsdafsddo()this is a test")), "this is a test");
    }
}
