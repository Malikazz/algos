...
fn main() {
    ...
    println!("{:?}", part_two(parse_input("src/input")));
}
...
fn part_two(values: Vec<(u128, Vec<u128>)>) -> u128 {
    let mut sum: u128 = 0;
    let maths = vec!["x", "+", "||"];
        ...
        for perm in repeat_n(maths.iter().cloned(), line.1.len() - 1).multi_cartesian_product() {
            ...
            for (index, operator) in perm.iter().enumerate() {
                match operator {
                    &"+" => current_sum += line.1[index + 1],
                    &"x" => current_sum *= line.1[index + 1],
                    &"||" => current_sum = (current_sum.to_string() + &line.1[index + 1].to_string()).parse::<u128>().unwrap(),
                    _ => panic!("should never reach here"),
                }
            }
            if current_sum == line.0 {
